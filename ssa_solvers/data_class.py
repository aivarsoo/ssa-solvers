...
class SimulationData:
        ...
        if self.save_to_file:
            ...
            for file_idx, file_ in enumerate(files):
                ...
                self._process_batch_trajectories(
                    raw_times_trajectories, raw_pops_trajectories, time_grid, write_header=file_idx == 0, runs_ids=runs_ids)
        else:
    ...
    def mean_and_std(self,  time_grid: Optional[List[int]] = None) -> torch.Tensor:
        ...
        if self.save_to_file:
            ...
            assert not (
                time_grid is None and self.processed_pops_trajectories is None), "Specify time_range as no data was processed"
